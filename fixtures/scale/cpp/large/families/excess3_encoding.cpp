...
std::string excess3_encoding_encode(const std::string& input) {
    ...
    for (std::size_t i = 0; i < input.size(); i += 2) {
        unsigned char hi = static_cast<unsigned char>(input[i] - '0' + 3);
        unsigned char lo = (i + 1 < input.size()) ? static_cast<unsigned char>(input[i + 1] - '0' + 3) : 3;
...
std::string excess3_encoding_decode(const std::string& input) {
    ...
    for (unsigned char byte : packed) {
        unsigned char hi = static_cast<unsigned char>(((byte >> 4) & 0x0F) - 3);
        unsigned char lo = static_cast<unsigned char>((byte & 0x0F) - 3);
...
bool excess3_encoding_verify() {
    ...
    std::string encoded = excess3_encoding_encode(sample);
    std::string decoded = excess3_encoding_decode(encoded);
