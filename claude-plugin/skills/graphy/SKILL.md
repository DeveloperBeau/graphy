...
1. **Stats first.** Call `stats` once at the start of any investigation so you know the graph's shape. If `nodes` is `0`, run `/graphy` to build the graph; the auto-build hook usually does this for you but a fresh checkout might not yet have a graph.
...
- `/graphy [path]` — explicit (re)build
- `/graphy-stats` — one-liner summary
- `/graphy-search <q>` — substring search
- `/graphy-neighbors <id>` — adjacency listing
- `/graphy-path <from> <to>` — shortest path
- `/graphy-report` — the markdown report

The MCP tools are always available regardless; the slash commands are wrappers that pre-format common workflows.
