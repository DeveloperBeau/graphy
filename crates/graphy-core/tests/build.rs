...
fn merges_disjoint_extractions() {
    let a = ExtractionOutput { nodes: vec![n("a")], edges: vec![] };
    let b = ExtractionOutput { nodes: vec![n("b"), n("c")], edges: vec![e("b", "c")] };
...
fn dedupes_nodes_across_extractions() {
    let a = ExtractionOutput { nodes: vec![n("x")], edges: vec![] };
    let b = ExtractionOutput { nodes: vec![n("x")], edges: vec![] };
    let g = build_graph(vec![a, b]);
...
fn edges_referencing_undeclared_nodes_create_them() {
    let a = ExtractionOutput { nodes: vec![], edges: vec![e("ghost1", "ghost2")] };
