//! Perl language plugin for graphy.
...
fn extract_to_json(path: &str, source: &str) -> Result<Vec<u8>, String> {
    ...
    parser
        ...
        .map_err(|e| format!("load tree-sitter-perl: {e}"))?;
    let tree = parser
        .parse(source, None)
        ...
        match child.kind() {
            "subroutine_declaration_statement" | "func_declaration_statement" => {
                if let Some(n) = name_of(child, src).or_else(|| first_id(child, src)) {
                    emit_def(out, symbols, file, "sub", n, child.start_position().row);
                }
            }
            "package_statement" | "package" => {
                if let Some(n) = name_of(child, src).or_else(|| first_id(child, src)) {
                    emit_def(out, symbols, file, "package", n, child.start_position().row);
                }
            }
