...
Hovering a node shows a tooltip with its full id (`app/util.py::say`, not just the short label) and every relation in both directions — `calls → say`, `← imported by hello.py`, and so on.
